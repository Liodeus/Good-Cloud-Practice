...
def gae_env_secret(cmd_list, report="False", lock="", severity="Critical", mitigation_name="gae_env_secret.md"):
	...
	"""
	env_secret = gae_reduce(cmd_list, "gae_env_secret", lock)
	env_secret_result = gae_reduce_two(env_secret, cmd_list, "gae_env_secret")
	report_print("GAE env variable check", env_secret_result, report, mitigation_name, severity, lock)
...
def gae_max_version(cmd_list, report="False", lock="", severity="Critical", mitigation_name="gae_max_version.md"):
	...
	"""
	max_version = gae_reduce(cmd_list, "gae_max_version", lock)

	if not len(max_version) > 2:
		max_version = {}

	report_print("GAE max version check", max_version, report, mitigation_name, severity, lock)
...
def gae_location(cmd_list, report="False", lock="", severity="Major", mitigation_name="gae_location.md"):
	...
	"""
	location = gae_reduce(cmd_list, "gae_location", lock)
	report_print("GAE location check", location, report, mitigation_name, severity, lock)
...
def gae_runtime(cmd_list, report="False", lock="", severity="Major", mitigation_name="gae_runtime.md"):
	...
	"""
	runtime = gae_reduce(cmd_list, "gae_runtime", lock)
	runtime_result = gae_reduce_two(runtime, cmd_list, "gae_runtime")
	report_print("GAE runtime check", runtime_result, report, mitigation_name, severity, lock)
